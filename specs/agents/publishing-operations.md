...
4. **Post Kit (the §12.3.1 handoff bundle, via `handoff_export`).** Build the per-piece kit under `/<brand>/handoff/<piece_id>/`: **zero-padded, carousel-ordered slide files**, the final caption in the correct language as a copy-block, first-comment hashtags as a separate copy-block, per-slide alt text, channel, a QR / send-to-phone link, a paste-order checklist, plus optional `location_tag` / `collaborator_handles`. The deterministic **platform-export pre-check** (§14.2) runs first — an over-limit caption/hashtag set or mixed aspect ratios blocks the kit with the exact failing limit named. The owner clicks, copies, pastes, posts — zero friction. `handoff_export` adds **no** publish authority.
5. **Queue to system of record.** Write to the Approval Queue in Sheets. Flag anything sitting in the queue >72h; a dated piece past `target_date` + grace gets `exception = Stale-Dated` and routes to the owner (Re-date / Publish anyway / Archive — never auto-published late).
   **Mark-as-posted closes the manual loop (§12.3.2):** after the owner posts manually, THEY record "Mark posted" (an Owner-Action value) with the permalink — `Approved → Published` with `posted_permalink`/`posted_at`. **Never flip a piece to Published yourself**; a manual piece without proof stays `posted_unverified` and is chased in the digest. If the underlying draft changed after the kit was built, the bundle is `handoff_bundle_stale` — regenerate before handing off.
